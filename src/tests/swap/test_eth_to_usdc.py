#!/usr/bin/env python3
"""
Test for ETH to USDC Swap

This module provides a test client for the ETH to USDC swap functionality.
...
def init_client():
    ...
    try:
        ...
        client_identity = Identity.from_seed("eth_to_usdc_test_seed_phrase", 0)
...
<description>Test client for Swapland ETH to USDC agent on Base network.</description>
<use_cases>
    <use_case>Tests the ETH to USDC swap functionality.</use_case>
</use_cases>
<payload_requirements>
<description>Expects a float value for the amount of ETH to convert to USDC.</description>
    <payload>
          <requirement>
...
"""
        ...
        register_with_agentverse(
            ...
            agent_title="Test Client for ETH to USDC Swap",
            readme=readme
        )
...
def send_request():
    """
    Send a test swap request to the ETH to USDC agent.
    ...
    try:
        ...
        swap_request = SwapRequest(
            ...
            signal=data.get("signal", "sell"),
            amount=float(data.get("amount", 0.1))
        )
        
        # Target agent address - the ETH to USDC agent
        target_agent = "agent1qfrhxny23vz62v5tr20qnmnjujq8k5t0mxgwdxfap945922t9v4ugqtqkea"
...
def run_test_server():
    ...
    logger.info("Test server running on http://localhost:5002")
    logger.info("Send POST requests to /send_request to test the ETH to USDC swap agent")
            ...
            if cmd.lower() == "exit":
                ...
                try:
                    ...
                    response = requests.post(
                        "http://localhost:5002/send_request",
                        json={
                            "blockchain": "base",
                            "signal": "sell",
                            "amount": float(amount)
                ...
                try:
                    import requests
                    response = requests.get("http://localhost:5002/last_response")
